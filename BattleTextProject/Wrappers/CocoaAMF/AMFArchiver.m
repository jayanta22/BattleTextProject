...
- (void)setClassName:(NSString *)codedName forClass:(Class)cls
{
	if (codedName == nil)
		...
		m_registeredClasses[cls] = codedName;
}
...
+ (void)setClassName:(NSString *)codedName forClass:(Class)cls
{
	...
	if (codedName == nil)
		...
		g_registeredClasses[cls] = codedName;
}

- (NSString *)classNameForClass:(Class)cls
{
	return m_registeredClasses[cls];
}

+ (NSString *)classNameForClass:(Class)cls
{
	return g_registeredClasses[cls];
}
...
- (void)_encodeASObject:(ASObject *)value
{
	...
	for (NSString *key in value.properties)
	{
		...
		NSObject *o = (value.properties)[key];
...
- (void)_encodeTraits:(AMF3TraitsInfo *)traits
{
	...
	{
		[self _encodeString:(traits.properties)[i] omitType:YES];
	}
}
