	...
	arguments:(NSObject *)arguments
{
	if (self == [self init])
	{	
		NSLog(@"AMFRemotingCall URLarguments %@",url);
		self.URL = url;
	...
	method:(NSString *)method arguments:(NSObject *)arguments
{
	...
	NSLog(@"AMFRemotingCall remotingCall %@",remotingCall);
...
{	
	if ([[(NSHTTPURLResponse *)response allHeaderFields][@"Content-Type"] 
		rangeOfString:@"application/x-amf"].location == NSNotFound)
	{
		m_error = [[NSError errorWithDomain:kAMFRemotingCallErrorDomain 
			code:kAMFInvalidResponseErrorCode userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"The server returned no application/x-amf data at URL %@.", 
					[[response URL] absoluteString]]}] retain];
	}
	else if ([(NSHTTPURLResponse *)response statusCode] != 200)
	{
		m_error = [[NSError errorWithDomain:kAMFRemotingCallErrorDomain 
			code:kAMFServerErrorErrorCode userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"The server returned status code %d at URL %@.", 
					[(NSHTTPURLResponse *)response statusCode], [[response URL] absoluteString]], kAMFServerStatusCodeKey: @([(NSHTTPURLResponse *)response statusCode])}] retain];
	}
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
	...
	{
		m_error = [[NSError errorWithDomain:kAMFRemotingCallErrorDomain 
			code:kAMFInvalidResponseErrorCode userInfo:@{NSLocalizedDescriptionKey: @"The server returned zero bytes of data"}] retain];
	}
	
	if (m_error)
	{	
		objc_msgSend(m_delegate, @selector(remotingCall:didFailWithError:), self, m_error);
		//NSLog(@"logggg %@",objc_msgSend(m_delegate, @selector(remotingCall:didFailWithError:), self, m_error));
	}
	else
	{
		AMFActionMessage *message = [[AMFActionMessage alloc] initWithData:m_receivedData];
		NSObject *data = [(message.bodies)[0] data];
