...
{
	return (m_currentDeserializedObject.properties)[key] != nil;
}
...
{
	return m_registeredClasses[codedName];
}

+ (Class)classForClassName:(NSString *)codedName
{
	return g_registeredClasses[codedName];
}
...
- (void)setClass:(Class)cls forClassName:(NSString *)codedName
{
	if (cls == NULL)
		...
		m_registeredClasses[codedName] = cls;
}
...
+ (void)setClass:(Class)cls forClassName:(NSString *)codedName
{
	...
	if (cls == NULL)
		...
		g_registeredClasses[codedName] = cls;
}
...
{
	return (m_currentDeserializedObject.properties)[key];
}
...
- (NSNumber *)_decodeNumberForKey:(NSString *)key
{
	NSNumber *num = (m_currentDeserializedObject.properties)[key];
...
- (NSObject *)_decodeASObject:(NSString *)className
{
	...
	m_objectTable[[m_objectTable indexOfObject:object]] = desObject;
	[object release];
	return desObject;
}
...
- (NSObject *)_decodeArray
{
	...
	for (;;)
	{
		...
		((NSMutableDictionary *)array)[name] = [self decodeObject];
	}
	...
	else
	{
		for (uint32_t i = 0; i < length; i++)
		{
			((NSMutableDictionary *)array)[[NSNumber numberWithInt:i]] = [self decodeObject];
		}
	}
